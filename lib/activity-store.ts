...
export type StoredActivity = {
  uid:string; id:number; version:number; lead:string; leadId?:string; type:string; detail:string; time:string; owner:string;
...
type ActivityRow = {
  uid:string; id:number; version:number; lead:string; lead_id:string|null; type:string; detail:string; time:string; owner:string; status:string;
...
export function mapActivity(row:ActivityRow):StoredActivity{
  return {uid:row.uid,id:row.id,version:Number(row.version||1),lead:row.lead,leadId:row.lead_id||undefined,type:row.type,detail:row.detail,time:row.time,owner:row.owner,status:(row.status||'Completed') as StoredActivity['status'],occurredAt:row.occurred_at||undefined,outcome:row.outcome||undefined,duration:row.duration||undefined,subject:row.subject||undefined,value:row.value||undefined,documentLink:row.document_link||undefined,attachmentKey:row.attachment_key||undefined,attachmentName:row.attachment_name||undefined,relatedTaskId:row.related_task_id??undefined,relatedTaskUid:row.related_task_uid||undefined,opportunity:row.opportunity||undefined,deletedAt:row.deleted_at||undefined};
}
