...
export function FinderWorkspace({ notify, leads, onImportedLeads, setPage, finderView, setFinderView }: Props) {
  ...
  const runSearch = async (searchId?: string) => {
    ...
    setBusy(true); setError(''); setImportSummary(null); setSelected([]); setResults([]);
    try {
      const body = searchId
        ? { action: 'run', searchId }
  ...
  const progressSteps = [
    { label: 'Discovering businesses', threshold: 18, detail: 'Google Places' },
    { label: 'Checking public details', threshold: 48, detail: 'Phones, websites and public contacts' },
    { label: 'Scoring best matches', threshold: 86, detail: 'Rules and Gemini assessment' },
  ...
  const historyTable = (items: FinderSearch[]) => <div className="finder-runs-table">
    ...
    {items.map((search) => <div className="finder-run-row" key={search.id}>
      ...
      <div className="finder-run-actions">
        {editingSearch === search.id ? <button onClick={() => updateSearch(search, 'rename', editingName.trim())} disabled={!editingName.trim()} aria-label="Save name"><FiCheck /></button> : <button onClick={() => openSearch(search)} aria-label="Open search"><FiChevronRight /></button>}
        <button onClick={() => runSearch(search.id)} aria-label="Run again"><FiRefreshCw /></button>
        <button onClick={() => duplicateSearch(search)} aria-label="Copy search"><FiCopy /></button>
        <button onClick={() => { setEditingSearch(search.id); setEditingName(search.name); }} aria-label="Rename search"><FiEdit3 /></button>
        <button onClick={() => updateSearch(search, search.saved ? 'unsave' : 'save')} aria-label={search.saved ? 'Remove from saved' : 'Save search'}><FiSave /></button>
        <button onClick={() => deleteSearch(search)} aria-label="Delete search"><FiTrash2 /></button>
      </div>
    </div>)}
  ...
  if (!activeSearch) return <div className="finder-workspace finder-setup">
    <header className="finder-page-head"><div><span className="eyebrow">Lead intelligence</span><h1>Find businesses worth contacting</h1><p>Scout discovers real businesses, verifies public details and ranks the strongest opportunities.</p></div><div className="finder-budget-pill"><FiZap /><span>AI spend</span><strong>${aiUsage.estimatedUsd.toFixed(3)} / ${aiUsage.budgetUsd.toFixed(2)}</strong></div></header>
    ...
    <section className="finder-search-card">
      <div className="finder-search-card-head"><div><span>New search</span><strong>Define your ideal business</strong></div><span>Google Places + public websites + Gemini</span></div>
  ...
  if (isRunning && !hasResults) return <div className="finder-workspace">
    <header className="finder-page-head"><div><span className="eyebrow">Finder is working</span><h1>{activeSearch.industry} in {activeSearch.location}</h1><p>This job continues safely in the background. You can leave this page and return later.</p></div><button className="secondary" disabled={busy} onClick={cancelSearch}>Cancel search</button></header>
    {error && <div className="finder-alert error"><FiAlertTriangle />{error}<button onClick={() => setError('')}><FiX /></button></div>}
  ...
  return <div className="finder-workspace finder-results-workspace">
    ...
    {error && <div className="finder-alert error"><FiAlertTriangle />{error}<button onClick={() => setError('')}><FiX /></button></div>}
    <section className="finder-kpis"><div><i className="blue"><FiUsers /></i><span><strong>{results.length}</strong>Businesses found</span></div><div><i className="green"><FiTarget /></i><span><strong>{qualified.length}</strong>Qualified</span></div><div><i className="violet"><FiPhone /></i><span><strong>{contactable}</strong>Contactable</span></div><div><i className="amber"><FiGlobe /></i><span><strong>{withWebsites}</strong>With websites</span></div><div><i><FiCheck /></i><span><strong>{duplicates}</strong>Already in Leads</span></div><div className="finder-cost"><span>Gemini usage</span><strong>${totalCost.toFixed(3)}</strong><small>${aiUsage.estimatedUsd.toFixed(3)} of ${aiUsage.budgetUsd.toFixed(2)} this month</small></div></section>
